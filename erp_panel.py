...
if page == "🏠 Ana Sayfa":
    ...
    with st.expander("➕ Yeni Parça Kaydet", expanded=True):
        with st.form("parca_form"):
            ...
            if st.form_submit_button("Parçayı Kaydet"):
                yeni_parca = {
                    "Varlık Etiketi": p_etiket, "Model": p_model, "Durum": p_durum, 
                    "Seri No": p_seri, "Son Tarih": p_tarih.strftime("%d-%m-%Y"),
                    "Yazılım Vers.": p_yazilim, "Bağlı Cihaz": p_bagli, "Renk": p_renk, 
                    "Tür": p_turu, "Notlar": p_notlar
                }
    ...
    if st.session_state.parca_listesi:
        # st.data_editor sayesinde liste üzerinde Excel gibi değişiklik yapabilirsin
        st.data_editor(pd.DataFrame(st.session_state.parca_listesi), use_container_width=True, num_rows="dynamic")
    else:
        st.info("Henüz kayıtlı parça yok.")
        ...
        with st.form("cihaz_form"):
            ...
            if st.form_submit_button("Cihazı Kaydet"):
                st.session_state.cihaz_listesi.append({
                    "Cihaz Adı": d_adi, "Seri No": d_seri, "Tür": d_turu, "Model": d_model,
                    "Sensör": d_sensor, "Anakart": d_anakart, "Modül": d_modul, "Durum": d_durum, "Notlar": d_notlar
                })
    ...
    if st.session_state.cihaz_listesi:
        ...
        st.info("Henüz kayıtlı cihaz yok.")