...
# ══════════════════════════════════════════
    ...
    if IS_YONETICI or IS_MUHENDIS:
        if "parca_form" not in st.session_state:
    st.session_state.parca_form = False

if st.button("Yeni Parça Ekle"):
    st.session_state.parca_form = True

if st.session_state.parca_form:
    with st.form("p_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Kaydet"):
            with conn.session as s:
                ...
            st.session_state.parca_form = False   # 🔥 BURASI ÖNEMLİ
...
# ══════════════════════════════════════════
    ...
    if IS_YONETICI or IS_MUHENDIS:
       if "cihaz_form" not in st.session_state:
    st.session_state.cihaz_form = False

if st.button("Yeni Cihaz Ekle"):
    st.session_state.cihaz_form = True

if st.session_state.cihaz_form:
    with st.form("cihaz_formu", clear_on_submit=True):
        ad = st.text_input("Cihaz Adı")
        ip = st.text_input("IP")

        if st.form_submit_button("Kaydet"):
            with conn.session as s:
                s.execute(text("INSERT INTO cihazlar (cihaz_adi, ip) VALUES (:ad,:ip)"),
                          {"ad": ad, "ip": ip})
                s.commit()
            st.session_state.cihaz_form = False
...
# ══════════════════════════════════════════
    ...
    with tab1:
     if "harcama_form" not in st.session_state:
    st.session_state.harcama_form = False

if st.button("Yeni Harcama Ekle"):
    st.session_state.harcama_form = True

if st.session_state.harcama_form:
    with st.form("harcama_formu", clear_on_submit=True):
        ...
        if st.form_submit_button("Kaydet"):
            with conn.session as s:
                s.execute(text("INSERT INTO harcamalar (tutar, kategori) VALUES (:t,:k)"),
                          {"t": tutar, "k": kategori})
                s.commit()
            st.session_state.harcama_form = False
            islem_basarili("Harcama eklendi")
...
# ══════════════════════════════════════════
   ...
   if "gorev_form" not in st.session_state:
    st.session_state.gorev_form = False

if st.button("Yeni Görev Ekle"):
    st.session_state.gorev_form = True

if st.session_state.gorev_form:
    with st.form("gorev_formu", clear_on_submit=True):
        baslik = st.text_input("Başlık")
        atanan = st.text_input("Atanan")

        if st.form_submit_button("Kaydet"):
            with conn.session as s:
                s.execute(text("INSERT INTO gorevler (baslik, atanan) VALUES (:b,:a)"),
                          {"b": baslik, "a": atanan})
                s.commit()
            st.session_state.gorev_form = False
...
# ══════════════════════════════════════════
    ...
    if "personel_form" not in st.session_state:
    st.session_state.personel_form = False

if st.button("Yeni Personel Ekle"):
    st.session_state.personel_form = True

if st.session_state.personel_form:
    with st.form("personel_formu", clear_on_submit=True):
        ...
        if st.form_submit_button("Kaydet"):
            with conn.session as s:
                s.execute(text("INSERT INTO personel (isim, email) VALUES (:i,:e)"),
                          {"i": isim, "e": email})
                s.commit()
            st.session_state.personel_form = False
